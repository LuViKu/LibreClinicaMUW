...
- [ ] **Triage current test suite**: 21 `*Test.java` files exist; verify they pass on `lc-develop`. Skip-list any flaky tests with `@Ignore("phase-0-triage: <reason>") ` and file an issue per skip.
- [ ] **Flip `<skipTests>true</skipTests>` → `false`** in [pom.xml](pom.xml) once green (until then CI passes `-DskipTests=false`).
