...
## Step 10 - Submitting a contribution

### Always branch from lc-develop
...
On GitHub, open a PR from your fork's branch targeting
`reliatec-gmbh/LibreClinica:lc-develop`. Include:

- A plain-English description of the problem
- Steps to reproduce (for bugs)
- What you changed and why
- `Closes #123` on its own line to auto-link the issue
...
| Error | Cause | Fix |
|-------|-------|-----|
| `mvn: command not found` | Maven not on PATH | Add `C:\tools\maven\bin` to System PATH and restart terminal |
| `JAVA_HOME not set` | Environment variable missing | Add `JAVA_HOME=C:\tools\jdk11` to System variables |
| `com/sun/codemodel/CodeWriter` | Java 11 used for ODM module | Switch `JAVA_HOME` to Java 8 for the build session |
| `zip file is empty` on any JAR | Corrupted download from jitpack.io | Delete the cached JAR from `.m2\repository\` and add jitpack mirror to `settings.xml` |
| `Non-parseable settings.xml` | BOM character added by Notepad | Recreate `settings.xml` using PowerShell `[xml]` method |
| `Cannot resolve Maven home` | IntelliJ cannot find Maven | Set Maven home path in **Settings -> Build Tools -> Maven** to `C:\tools\maven` |
| `401 Unauthorized` from JFrog | Private repository access | Add JFrog mirror to `settings.xml` redirecting to Maven Central |
| Tomcat deployment error on first start | Missing `datainfo.properties` | Create `libreclinica.config` folder and copy template config file |
| `lc-develop` branch not found | Cloned from fork before branching | Run `git fetch upstream` then `git checkout lc-develop` |

---

## Port reference

When running LibreClinica alongside other local development stacks:

| Service | Default port | Notes |
|---------|-------------|-------|
| Tomcat (LibreClinica) | 8080 | No conflict with WAMP/XAMPP |
| PostgreSQL | 5432 | No conflict with MySQL (3306) |
| WAMP Apache | 80 | Separate stack, leave untouched |
| MySQL (WAMP) | 3306 | Separate stack, leave untouched |

All four services can run simultaneously without port conflicts.
