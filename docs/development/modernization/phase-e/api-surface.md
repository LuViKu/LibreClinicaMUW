...
| 17 | [dm-import-crf-data](ux-mockups/dm-import-crf-data.html) | `/ImportCRFData` | **B** + **C** | Wizard. **Phase E.6 bulk-import (shipped) implements:** `POST /pages/api/v1/import` (multipart .xml → `ImportCrfPreviewDto` with preview-token + first 200 rows); `GET /pages/api/v1/import/{token}/rows?offset=&limit=` (windowed `PreviewRowsPageDto`); `POST /pages/api/v1/import/commit` (body `{ previewToken, reasonForChange?, overwriteMode? }` → `ImportCrfCommitResult`). Auth: sysadmin OR Director/Coordinator/Investigator/RA on the active study. Persistence path (real commit) deferred — the controller currently returns 501 from `/commit` with a deterministic `PENDING_PERSISTENCE_EXTRACTION` status. |
