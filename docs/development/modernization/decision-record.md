...
- The storage panel names every store with its path, which makes one finding visible: the app's data directory (`filePath`: CRF attachments and dataset exports) is an **anonymous Docker volume** declared by the Dockerfile, not a bind mount under `/var/lib/libreclinica`. It survives an image upgrade (Compose re-attaches anonymous volumes on recreate) but not a `docker compose down` followed by `up`, and it lies outside the backup root. Moving it to a bind mount (copying the current contents out first) is a deployment change, recorded as a follow-up rather than made here.
