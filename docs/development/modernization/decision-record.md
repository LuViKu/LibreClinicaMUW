...
- **Addendum 2026-09-24 — blinding is a per-study setting.** The rule above was written for nAMD, where a treating physician seeing fluid volumes would influence the decision the trial measures. HealthAEye is a screening study: no treatment arm, no such decision, and the physician *is* the reader of the AI output — yet the metrics link on a visit bounced them to the home page. `ai.blinding.enabled` (default **on**, so no existing study changes) now says which kind of study this is. Off, the arm lookups in `AiArmPolicy` resolve to "no arm" for every subject — the one answer masking never triggers on — so the seven server-side masking sites follow without being touched, and the SPA's route guard lifts the treating roles' exclusion on the two metrics routes through the same setting as `/me` reports it. Nothing else the setting is not named on changes; the default stays fail-closed.
...
## DR-033 — The System Status page sees the uploaders on the acquisition PCs and the storage the platform uses

**Date:** 2026-09-24
**Status:** Accepted
**Owner:** Lead Developer (Lukas Kuchernig)
**Related:** DR-025 (Optomed bridge), DR-029 (the one front door), DR-032 (Export Watcher); `UploaderHeartbeatApiController`, `SystemHealthApiController`, `StorageUsageSampler`, `DirectoryUsage`; migration `lc-muw-2026-12-05-uploader-health-storage.xml`; `deploy/export-watcher/ExportWatcher.ps1`, `deploy/optomed/OptomedBridge.ps1`.

**Context.** Three programs on two Windows PCs now carry HealthAEye images to the platform: the Export Watcher on the Clarus PC and on the Spectralis PC, and the Optomed Bridge beside the Optomed Client. Nothing on the platform shows whether they run. A watcher that was closed, a PC that lost the network, an Optomed Client that refused a worklist file: each looks, from the server, exactly like a quiet clinic morning, and the photographer finds out days later from a missing image. The storage side is as blind: OCT volumes, fundus images, segmentation results and their B-scans grow by gigabytes a week, and the first sign of a full disk would be failed ingests.

**Decision.**

1. **Each program reports; the platform does not poll.** The platform cannot open connections into the clinic PCs, and the programs already reach its upload front door. Every 120 s — switched on or off, because "switched off" is one of the things to see — each posts one small JSON document to `POST /api/v1/device/uploader/heartbeat`: running or not, uploading on or not, files waiting and how long the oldest has waited, files given up on (`_failed\`), free and total space on the export drive, and coded problems from its last round (platform unreachable, rate-limited, upload failed, unreadable files, export folder missing; for the bridge also the worklist refused by the Client, a rejected token, the worklist switched off, the Client not running). Times travel as ages in seconds, never as clock readings: an acquisition PC's clock is not the server's. No label, no filename, nothing about a patient. A program closed from its menu, or by Windows ending the session, says so in a last heartbeat, so the page can tell a PC switched off in the evening from a crash.
2. **Posture of the endpoint.** Unauthenticated, like the front door the same programs use; nothing is entered on those PCs but a URL, and the heartbeat carries no patient data. Its row is keyed by a random instance id the program generated on first start and that the page never shows, so knowing a PC's name is not enough to report on its behalf. Every field is bounded and read leniently (a malformed optional field is dropped, not fatal); the body is capped at 16 KB before parsing; at most 50 programs are admitted, beyond which a new one gets 429 while known ones keep reporting; heartbeats have their own per-client budget (240/h) apart from the upload page's lookups; `core.uploaderHealth.heartbeat.enabled=false` turns the endpoint into a 404.
3. **What the page makes of it** (`GET /api/v1/admin/uploaders`, sysadmin only). *Stopped* (the program said so, with the reason) outranks *offline* (no heartbeat for three intervals, never sooner than five minutes); then *switched off*; then *needs attention* when the program reported a problem or the server sees one of its own — files in `_failed\`, a file waiting 30 minutes or more while uploading is on, less free disk than the larger of 5 GiB and 5 %; else *OK*. A second table beside it counts what actually arrived per device and way in over 90 days, from `ingest_item`: a program can report itself healthy while the export goes into another folder, and files can arrive through the page while the program is dead. A sysadmin can remove a program's row; one that still runs re-appears with its next heartbeat.
4. **Storage is measured hourly, not on a page load.** Walking the stores is a metadata read per file — seconds to minutes on a year of images — so `StorageUsageSampler` walks them once an hour (and on demand), within a budget of five million files and fifteen minutes per store, a store inside another counted once. It records, per store, bytes and files and the filesystem under it with its free space, plus the database size, into `storage_usage_sample`, kept for 90 days. `GET /api/v1/admin/storage` shows the newest scan, the change against the scan closest to a week earlier, and — where free space shrank — the days until the disk is full at that rate. A store on the container's own layer (overlay) is flagged: it does not survive the container being recreated.

**Consequences.**

- Two additive tables. `uploader_instance` holds the last heartbeat per program, not a history; `storage_usage_sample` holds about (stores + 1) × 24 × 90 rows.
- Each script carries a `$script:Version` shown on the page, to be bumped with every change, so a PC running an old copy stands out. Both gain a *Name on the status page* setting (default: the computer name) and a headless `-Heartbeat` check for installation. The watcher's folder walk no longer enters `_uploaded\` and `_skipped\`: they only grow, and the sweep listed them every twenty seconds.
- The storage panel names every store with its path, which makes one finding visible: the app's data directory (`filePath`: CRF attachments and dataset exports) is an **anonymous Docker volume** declared by the Dockerfile, not a bind mount under `/var/lib/libreclinica`. It survives a Compose recreate, which re-attaches anonymous volumes, but the production unit restarts with `docker compose down` + `up`: every restart, each upgrade and each reboot, gave the app fresh, empty volumes and left the previous ones behind. After the beta.11 upgrade the VM held 82 of them (5.1 GB, old Tomcat logs only: no attachment or export had been written yet). **Corrected 2026-09-24** (PR after #341): both directories are bound from `/var/lib/libreclinica/app-data` and `/var/lib/libreclinica/tomcat-logs`, and the setup script copies the running container's current contents across once before the restart.
- Found while wiring the heartbeat's budget, and left for its own change: the public rate limit in `PublicOctUploadRateLimitFilter` never applies in the deployed app. It matches its prefixes against the request URI, which carries the `/LibreClinica` context path the prefixes do not; 36 lookups in a row all answered 200 on the dev stack. Separately, `/resolve` takes any number of labels per request. The heartbeat's protections above do not rely on the filter; switching the limit on changes what the Export Watcher and the bridge see under load (their `/resolve` calls would start to meet 429), so it needs its own decision.
- Not covered: the backup directory and the GPU host's disks (neither is mounted into the app), and the acquisition PCs' disks beyond the export drive.
- Verified 2026-09-24 on the dev stack, the PowerShell side from PowerShell 7 in a container on the compose network: the watcher's `-Heartbeat` and `-Once` registered it and reported one upload, nothing pending and one file in `_failed\` (page: *needs attention*, files in `_failed`); the bridge's `-Heartbeat` reported one pulled image waiting 223 minutes and no Optomed Client (page: both problems named); a hand-sent stop heartbeat with `session-end` read as *logged off or shut down* and was removed through the page. The first storage scan measured six stores on two filesystems in 263 ms. The page passes the WCAG 2.2 AA check of the a11y suite with both panels. The tray parts (timers, Exit, the session-end event) run only on Windows and were not exercised.

**Reversible** — drop the two tables and the endpoints; the scripts log one line when the platform answers 404 and carry on.
