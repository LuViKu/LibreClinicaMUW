...
- **Conditional-reason display in non-runtime surfaces** — the active/inactive state machine for the OPHTH "Spectralis-OCT — Grund" input (and similar conditional-reason items) only resolves correctly inside the runtime CRF entry view. In the Authoring Wizard's Sektionen editor + live preview the row currently renders stuck in the inactive grey state regardless of the parent _DONE value, and the hint placeholder reads "Aktiv, sobald „" gewählt ist" with the trigger token missing. Runtime data entry is unaffected. Deferred to a follow-up — the parent-value resolver needs to be ported into the authoring-side widget host. Use the runtime CRF entry view to verify conditional behaviour in the meantime.
