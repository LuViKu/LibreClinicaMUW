...
- **The bridge also keeps the Optomed Client itself out of the way — and that is visible.** It starts the Client if it is not running, and it **minimises the Client's window and removes its taskbar button**, so the bridge's tray icon is the only sign of either. An operator who does not know this will think the Client has crashed: it has not, and **Show Optomed Client** in the bridge's menu brings it back. Minimised rather than hidden, because hiding that window from outside blanks its display for good — learned on the real device. A minimised, buttonless Client also cannot show its own prompts (a firmware update, a pairing error); Show is the answer there too.
...
- A bridge that is *killed* rather than exited from its menu leaves the Optomed Client minimised with no taskbar button until the bridge runs again (a clean Exit restores it). Starting the bridge, or its Show item, is the recovery; nothing is lost.
