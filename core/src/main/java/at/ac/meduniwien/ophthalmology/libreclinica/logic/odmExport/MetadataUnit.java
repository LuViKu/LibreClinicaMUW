...
import at.ac.meduniwien.ophthalmology.libreclinica.bean.odmbeans.MetaDataVersionIncludeBean;
...
import at.ac.meduniwien.ophthalmology.libreclinica.dao.extract.OdmExtractDAO;
...
public class MetadataUnit extends OdmUnit {
    ...
    public MetadataUnit(DataSource ds, StudyBean study, int category) {
        ...
        if (study.getParentStudyId() > 0) {
            this.parentStudy = new StudyDAO(ds).findByPK(study.getParentStudyId());
        } else {
    ...
    public static LinkedHashMap<String, String> parseCode(String rsText, String rsValue) {
        ...
        for (int i = 0; i < values.length; i++) {
            // Allow "" values as pseudo null value with no mandatory text
            // entry. This void avoid discrepancy within ODM export code list.
            if (values[i] == null) {
                continue;
            }
