...
/**
 * P3.6 — the guard surface of the artifact stream, moved with the endpoint.
 *
 * <p>The name in the path is attacker-controlled and is used to build a
 * filesystem path, so the allow-list refusing anything outside it is the
 * assertion worth keeping close to the code it protects. These cases need no
 * database: they are refused before anything is read.
 */
class RetinalJobArtifactsApiControllerTest extends AbstractApiControllerTest {

    private MockMvc mockMvcWith() {
        return mockMvcFor(new RetinalJobArtifactsApiController(
                mockDataSource(),
