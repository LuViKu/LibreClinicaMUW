...
import { useOctPortalStore, type ReviewRow as ReviewRowData } from '@/stores/octPortal'
...
/**
 * 2026-06-19 — real upload-progress driver. Reads the per-row percent
 * from the store's {@code uploadPct} Map (populated by XHR's
 * {@code upload.onprogress} in commitScan). Falls back to 0 % when
 * the row hasn't started uploading yet.
 *
 * <p>The {@code muw-portal-fill} + {@code muw-portal-fill-edge}
 * primitives in style.css consume the percent through the
 * {@code --muw-portal-fill-pct} CSS custom property. We show the
 * fill ONLY while the row is in {@code committing} — on success the
 * store advances to {@code committed} and the fill disappears; on
 * 409 / 5xx the store flips to {@code error} and the fill is also
 * dropped. The earlier deterministic JS timer painted a fake fill
 * that diverged from reality + kept growing on errors.
 */
const showFill = computed(() => props.row.state === 'committing')
const uploadPct = computed<number>(
  () => store.uploadPct.get(props.row.rowId) ?? 0,
)
const fillStyle = computed(() => ({
  '--muw-portal-fill-pct': `${uploadPct.value}%`,
}))
...
<template>
  ...
  >
    <!--
      2026-06-19 — committing-phase L→R green fill. The .muw-portal-fill
      block paints a teal-tinted band whose width is bound to the
      --muw-portal-fill-pct custom property (JS-driven). The
      .muw-portal-fill-edge is the 2 px progress edge. Both sit at z-0
      so the row content (z-10 on cells) reads on top.
    -->
    ...
    <!-- assignment + action — wrapped in a positioned container so the
         row's z-0 fill primitives stay behind the content. -->
