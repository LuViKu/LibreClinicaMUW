...
export function canCancelEvent(role: UserRole, status: StudyEventStatus): boolean {
  if (status === 'signed' || status === 'locked') return false
  // Writers who may edit a visit may also cancel it (with a reason)
  // per MUW workflow; structured cancel-reason capture still gates the
  // call for audit accountability.
