...
import at.ac.meduniwien.ophthalmology.libreclinica.bean.submit.crfdata.SummaryStatsBean;
...
public class ImportCRFDataService {
    ...
    private void attachValidator(DisplayItemBean displayItemBean, ImportHelper importHelper, DiscrepancyValidator v, HashMap<String, String> hardv,
        ...
        at.ac.meduniwien.ophthalmology.libreclinica.bean.core.ResponseType rt = displayItemBean.getMetadata().getResponseSet().getResponseType();
        ...
        if (rt.equals(at.ac.meduniwien.ophthalmology.libreclinica.bean.core.ResponseType.TEXT) || rt.equals(at.ac.meduniwien.ophthalmology.libreclinica.bean.core.ResponseType.TEXTAREA)) {
            ...
            // what if it's a number? should be only numbers
                ...
                try {
                    Integer.valueOf(displayItemBean.getData().getValue());
                    int width = Validator.parseWidth(widthDecimal);
                    if (width > 0 && displayItemBean.getData().getValue().length() > width) {
                        hardv.put(itemOid, "This value exceeds required width=" + width);
                    }
            ...
            // what if it's a float? should be only numbers
                ...
                try {
                    Float.valueOf(displayItemBean.getData().getValue());
                    int width = Validator.parseWidth(widthDecimal);
                    if (width > 0 && displayItemBean.getData().getValue().length() > width) {
                        hardv.put(itemOid, "This value exceeds required width=" + width);
                    }
