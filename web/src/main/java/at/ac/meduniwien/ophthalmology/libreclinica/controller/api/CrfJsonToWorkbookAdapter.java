...
public class CrfJsonToWorkbookAdapter {
    ...
    static {
        ...
        m.put("BL", "BL");
        m.put("BOOLEAN", "BL");
        // Map authoring-only TRISTATE_REASON → legacy ST; the runtime widget keys the
        // segmented yes/no/unknown rendering off the response set, not the dataType, so
        // the ST projection is lossless for the runtime path.
