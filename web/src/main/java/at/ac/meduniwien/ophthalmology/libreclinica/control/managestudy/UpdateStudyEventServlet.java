...
public class UpdateStudyEventServlet extends SecureController {
    ...
    public void processRequest() throws Exception {
        ...
        // YW 11-07-2007, a study event could not be updated if its study subject has been removed
        // Status s = ((StudySubjectBean)new StudySubjectDAO(sm.getDataSource()).findByPK(studySubjectId)).getStatus();
        ...
        ArrayList<SubjectEventStatus> statuses = SubjectEventStatus.toArrayList();
        ...
        if (removeSign || !currentRole.isInvestigator()) {
        ...
        ArrayList<EventCRFBean> getECRFs = studyEvent.getEventCRFs();
        // above removed tbh 102007, require to get all definitions, no matter if they are filled in or now
        EventDefinitionCRFDAO edefcrfdao = new EventDefinitionCRFDAO(sm.getDataSource());
        ArrayList<EventDefinitionCRFBean> getAllECRFs = edefcrfdao.findAllByDefinition(studyBean, studyEvent.getStudyEventDefinitionId());
        ...
        for (EventDefinitionCRFBean ecrfBean : getAllECRFs) {
            ...
            for (EventCRFBean existingBean : getECRFs) {
                logger.debug("***** found: {} {} {} {} {}",
                        existingBean.getCRFVersionId(),
                        existingBean.getCrf().getId(),
                        existingBean.getCrfVersion().getName(),
                        existingBean.getStatus().getName(),
                        existingBean.getStage().getName()
                );
                logger.debug("***** comparing above to ecrfBean.DefaultVersionID: {}", ecrfBean.getDefaultVersionId());

                // if (existingBean.getCRFVersionId() == ecrfBean.getDefaultVersionId()) {
                // OK. this only works if we go ahead and remove the dropdown
                ...
                // whatsoever this is addressed in the if-clause above
                if (!existingBean.getStatus().equals(Status.UNAVAILABLE) && edefcrfdao.isRequiredInDefinition(existingBean.getCRFVersionId(), studyEvent)) {
                    ...
                    logger.debug("found that {} is required...", existingBean.getCrfVersion().getName());
        ...
        if (action.equalsIgnoreCase("submit")) {
            ...
            if (!errors.isEmpty()) {
                ...
                if (fp.getString(INPUT_STARTDATE_PREFIX + "Hour").equals("-1") &&
                    ...
                    fp.getString(INPUT_STARTDATE_PREFIX + "Half").isEmpty()) {
                ...
                if (!strEnd.isEmpty()) {
                    ...
                    if (fp.getString(INPUT_ENDDATE_PREFIX + "Hour").equals("-1") &&
                        ...
                        fp.getString(INPUT_ENDDATE_PREFIX + "Half").isEmpty()) {
                ...
                ArrayList<EventDefinitionCRFBean> eventDefinitionCRFs = edcdao.findAllActiveByEventDefinitionId(study, studyEvent.getStudyEventDefinitionId());
                ...
                List<DisplayStudyEventBean> displayEvents = new ArrayList<>();
                // Set up a Map for the JSP view, mapping the eventCRFId to
                // another Map: the inner Map maps the resolution status name to the number of
                // notes for that eventCRF id, as in New --> 2
            ...
            // String encodedUserPass = at.ac.meduniwien.ophthalmology.libreclinica.core.SecurityManager.getInstance().encrytPassword(password);
            ...
            if (securityManager.verifyPassword(password, getUserDetails()) && ub.getName().equals(username)) {
                ...
                // If all the StudyEvents become signed we will make the StudySubject signed as well
                ArrayList<StudyEventBean> studyEvents = sedao.findAllByStudySubject(ssub);
            ...
            ArrayList<DiscrepancyNoteBean> allNotesForSubjectAndEvent = new ArrayList<>();
            ...
            if (studyEvent.getStartTimeFlag()) {
                ...
                // Later it could be put to somewhere as a static method if necessary.
	...
	private ArrayList<DisplayEventDefinitionCRFBean> getUncompletedCRFs(ArrayList<EventDefinitionCRFBean> eventDefinitionCRFs, ArrayList<EventCRFBean> eventCRFs) {
         ...
         * event definition ED, if (!isCompleted(ED)) { answer += ED; } returr answer;
         * This algorithm is guaranteed to find all the event definitions for which no event CRF exists.
    ...
    private void populateUncompletedCRFsWithCRFAndVersions(ArrayList<DisplayEventDefinitionCRFBean> uncompletedEventDefinitionCRFs) {
        ...
        for (int i = 0; i < uncompletedEventDefinitionCRFs.size(); i++) {
            ...
            // note that we do not check status in the above query, so let's check it here, tbh 102007
            if (cb.getStatus().equals(Status.AVAILABLE)) {
                // the above does not allow us to show the CRF as a thing with
                // status of 'invalid' so we have to go to the JSP for this one, I think
