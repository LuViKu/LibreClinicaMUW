 ...
 * pulls, the "today's visits" picker on the upload page, the due-visits view,
 * and the subject page's "is this patient on the camera today" strip. They
 * must agree: a visit the camera can photograph but the upload page will not
 * offer, or the reverse, is a defect the operator hits with a patient in the
 * chair.
 *
 ...
 *   <li>An optional study scope. Both device-facing callers serve devices
 *       that must not see other studies' patients; see {@link StudyScopeConfig}.</li>
 * </ul>
 *
...
final class ScheduledVisitQuery {
    ...
    /**
     * Everything that makes a visit "open" lives here, once. The two entry
     * points below only add the predicate that says <em>which</em> open visits
     * — a date window, or one subject.
     */
    private static final String OPEN_VISITS =
            "SELECT se.study_event_id, ss.study_subject_id, ss.label, ss.study_id, "
            ...
            + " WHERE se.subject_event_status_id IN (1, 3) "
            + "   AND ss.status_id NOT IN (5, 7) ";

    private static final String ORDERED = " ORDER BY se.date_start, ss.label ";

    /**
     * The open visits in a date window, across the studies in scope.
     *
    ...
    static List<ScheduledVisit> query(DataSource dataSource, LocalDate from, LocalDate to,
        ...
        String sql = OPEN_VISITS
                + "   AND date(se.date_start) BETWEEN ? AND ? "
                + (scope == null ? "" : "   AND ss.study_id IN " + scope + " ")
                + ORDERED
                + " LIMIT " + Math.max(1, limit);
        return run(dataSource, sql, ps -> {
    ...
    /**
     * Every open visit of one subject, whatever its date.
     *
     * <p>The subject page asks "is this patient on the camera today", and the
     * useful answer when not is <em>why</em> — the visit sits on another day,
     * or there is none. So this is the same statement without the window; the
     * caller splits today from the rest. A visit with no start date at all is
     * included (it is open, and it is never on a worklist), so the page can
     * offer to date it.
     */
    static List<ScheduledVisit> openVisitsOf(DataSource dataSource, int studySubjectId, int limit)
            throws SQLException {
        String sql = OPEN_VISITS
                + "   AND ss.study_subject_id = ? "
                + ORDERED
                + " LIMIT " + Math.max(1, limit);
        return run(dataSource, sql, ps -> ps.setInt(1, studySubjectId));
    }
    ...
    private static List<ScheduledVisit> run(DataSource dataSource, String sql, Binder binder)
            throws SQLException {
    ...
    private static ScheduledVisit read(ResultSet rs) throws SQLException {
