...
<c:choose>
 ...
 <!-- BWP 3098 >> switch displays for Info box-->
	...
	<c:if test="${iconInfoShown}">
		<!-- condition 1 ${iconInfoShown} -->
		<jsp:include page="../include/sideIconsSubject.jsp"/>	 	
	</c:if>
  <!-- BWP 3098: show icons by default; added   || closeInfoShowIcons-->
    ...
    <!-- condition 2 ${(!panel.iconInfoShown && panel.manageSubject) || closeInfoShowIcons}-->
	 <jsp:include page="../include/sideIconsSubject.jsp"/>
	</c:if>
	...
	<c:when test="${panel.submitDataModule}">      
    	<jsp:include page="../include/submitDataSide.jsp"/>
	</c:when>
  </c:choose>	
</c:when>
<c:otherwise>
